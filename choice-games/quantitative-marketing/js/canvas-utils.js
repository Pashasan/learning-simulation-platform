...
class CanvasPlotter {
    ...
    constructor(canvasId, options = {}) {
        ...
        // Default colors
        this.colors = {
            background: options.backgroundColor || '#141428',
            grid: options.gridColor || 'rgba(255,255,255,0.1)',
            axis: options.axisColor || '#fff',
            text: options.textColor || '#fff',
            point: options.pointColor || 'rgba(212, 160, 60, 0.7)',
            pointStroke: options.pointStrokeColor || 'rgba(212, 160, 60, 1)',
            line: options.lineColor || '#6c8ebf',
            ...options.colors
        };
    ...
    drawConfidenceBand(intercept, slope, width, options = {}) {
        const color = options.color || 'rgba(212, 160, 60, 0.2)';
        const [xMin, xMax] = this.xRange;
